      ...
      uploadImageStorage().then((response) => {
        ...
        db.collection("restaurants")
          ...
          .catch(() => {
            setIsLoading(false);
            toastRef.current.show(
              "Error al subir el restaurante, inténtelo mas tarde"
            );
          });
      });
    }
...
function UploadImage(props) {
  ...
  const imageSelect = async () => {
    ...
    if (resultPermissions === "denied") {
      toastRef.current.show(
        "Es necesario aceptar los permisos de garlería, si los haz rechazado tienes que ir a ajustes y activarlos manualmente.",
        3000
      );
    } else {
